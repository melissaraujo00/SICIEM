...
<table border="1" bgcolor="white" align="center">
	...
	<img src="imagenes/busqueda.ico" width="50" height="50"></h2></center>
			...
			<input type="text" name="codigo_inventario" placeholder="00000-0000-0000" value="<?php echo @$_POST['codigo_inventario']?>" required autofocus>
	...
	$consulta_mobiliario = mysqli_query($conexion, "SELECT * FROM mobiliarioyequipo WHERE codigo_inventario LIKE '%$codigo_inventario%'
		OR codigo_inventario LIKE '%$codigo_inventario%'
		OR CONCAT(codigo_inventario,'', codigo_inventario) LIKE '%$codigo_inventario'");
	$cantidad_registro = mysqli_num_rows($consulta_mobiliario);

	if ($cantidad_registro === 0) {
		echo "<tr><td><font color='red' size='4'>No hay registro con el codigo: ".$codigo_inventario."</font></td></tr>";
	} else { ?>
	<tr >
		<td>N°</td>
	...
	<?php
		while($mobiliario = mysqli_fetch_array($consulta_mobiliario)){
			...
			echo "<tr bgcolor =".$color.">";
				echo "<td>".$contador. "</td>";
				echo "<td bgcolor='yellow'><b>".$mobiliario['codigo_inventario']. "</td>";
