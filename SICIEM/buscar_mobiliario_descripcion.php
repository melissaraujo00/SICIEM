...
<table border="1" bgcolor="white" align="center">
	...
	$consulta_mobiliario = mysqli_query($conexion, "SELECT * FROM mobiliarioyequipo WHERE descripcion LIKE '%$descripcion%'
		OR descripcion LIKE '%$descripcion%'
		OR CONCAT(descripcion,'', descripcion) LIKE '%$descripcion'");
	$cantidad_registro = mysqli_num_rows($consulta_mobiliario);

	if ($cantidad_registro === 0) {
		echo "<tr><td><font color='red' size='4'>No hay registro con el codigo: ".$descripcion."</font></td></tr>";
	} else { ?>
	<tr >
		<td>N°</td>
